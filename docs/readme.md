...
| `otlp-trace`            | Deploy a second copy of the music application from its agentless images: the Java, Node.js and Python services instrumented at start by the OpenTelemetry auto-instrumentation, the APISIX frontend by its built-in `opentelemetry` plugin, all exporting OTLP traces to an OpenTelemetry Collector that delivers every span to the OAP both natively as OTLP and converted to Zipkin, so Grafana Tempo serves the same traces at `/otlp` and `/zipkin`. | Runs beside the SkyWalking-agent copy, in its own namespace on Kubernetes and its own network on Docker Compose, so both flows show at once. The same Selenium load generator drives the frontend page, so the traces start at `frontend`; the page carries no browser agent, and the Go `rating` service has no runtime instrumentation and appears only as a callee. The `grafana` feature provisions the `OTLPTraceQL` datasource and the `OTLP Trace` dashboard. |
