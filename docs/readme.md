...
| `otlp-trace`            | Deploy a second copy of the music application from its agentless images: the Java, Node.js and Python services instrumented at start by the OpenTelemetry auto-instrumentation, the APISIX frontend by its built-in `opentelemetry` plugin, every span exported straight to the OAP and stored natively, so Grafana Tempo serves this copy at `/otlp`. | Runs beside the SkyWalking-agent copy, in its own namespace on Kubernetes and its own network on Docker Compose, so both flows show at once. No collector sits in the trace path: the senders that can speak gRPC use port 11800, the Python agent and the APISIX plugin are HTTP-only and use OTLP/HTTP on 12800. `/zipkin` stays the mesh's, fed by Istio's sidecars. The same Selenium load generator drives the frontend page, so the traces start at `frontend`; the page carries no browser agent, and the Go `rating` service has no runtime instrumentation and appears only as a callee. The `grafana` feature provisions the `OTLPTraceQL` datasource and the `OTLP Trace` dashboard. |
